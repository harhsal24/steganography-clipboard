...
async function main() {
    ...
    for (const node of imageNodes) {
        // MODIFIED: Generate XPath from the path array for each node
        const xpath = convertToXPath(node.pathArray);
        const filename = node.value;
        
        if (typeof filename !== 'string' || filename.trim() === '') {
            console.warn(`\nSkipping invalid entry with XPath: ${xpath}`);
            continue;
        }
        
        const imagePath = path.join(imagesFolderPath, filename.trim());
        // MODIFIED: Update console log to show XPath
        ...
        try {
            ...
            if (result.success) {
                ...
                // MODIFIED: Use XPath in the output line
    ...
    // ... (no changes in the file writing part) ...
    try {
